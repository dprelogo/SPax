...
class Fisher():
    ...
    def fit(self, data, derivatives, δθ, batch_size = None):
        ...
        if self.devices is None:
            ...
            dμ_dθ = jnp.mean((derivatives[:, 1, ...] - derivatives[:, 0, ...]) / δθ, axis = -1, dtype = jnp.float64).astype(jnp.float32)
            
            self.UtJ = jnp.einsum("ki,kj->ij", pca.U, dμ_dθ, precision = jax.lax.Precision.HIGH).astype(jnp.float32)
        else:
            ...
            def partial_dμ_dθ(der):
                return jnp.mean((der[:, 1, :] - der[:, 0, :]) / δθ, axis = -1, dtype = jnp.float64).astype(jnp.float32)
            derivatives = derivatives.reshape(N_dim // (n_d * batch_size), n_d, batch_size, 2, len(δθ), N_samples)
            dμ_dθ = jnp.concatenate(jnp.array([jnp.concatenate(partial_dμ_dθ(x), axis = 0) for x in derivatives], dtype = jnp.float32), axis = 0)
    ...
    def compute(self, N = None, return_matrix = False):
        ...
        F = jnp.einsum("ki,k,kj->ij", self.UtJ[:N], self.Σ_inv[:N], self.UtJ[:N], precision = jax.lax.Precision.HIGH).astype(jnp.float32)
